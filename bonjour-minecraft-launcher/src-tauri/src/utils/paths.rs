...
pub fn validate_path_within_root(target: &Path, game_root: &Path) -> Result<PathBuf, AppError> {
    let game_root_canonical = game_root.canonicalize()
        .map_err(|e| crate::errors::internal(format!("Failed to resolve game root: {}", e)))?;

    let target_canonical = if target.exists() {
        target.canonicalize()
            .map_err(|e| crate::errors::internal(format!("Failed to resolve path: {}", e)))?
    } else {
    ...
    if !target_canonical.starts_with(&game_root_canonical) {
        return Err(crate::errors::invalid_param(
            "path",
            "Path traversal detected: target is outside game directory",
        ));
    }
...
pub fn detect_game_root(selected_dir: &str) -> String {
    if selected_dir.is_empty() {
        return default_game_dir().to_string_lossy().to_string();
    }
