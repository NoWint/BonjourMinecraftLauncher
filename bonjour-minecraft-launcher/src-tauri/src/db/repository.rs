...
use crate::utils::crypto::{encrypt_field, decrypt_field};
...
impl AccountRepository {
    ...
    pub fn get_all(&self) -> Result<Vec<Account>, AppError> {
        ...
        for row in rows {
            let mut account = db_op!(row, row)?;
            account.access_token = account.access_token.as_deref().map(|v| decrypt_field(v).unwrap_or_else(|_| v.to_string())).or(account.access_token);
            account.refresh_token = account.refresh_token.as_deref().map(|v| decrypt_field(v).unwrap_or_else(|_| v.to_string())).or(account.refresh_token);
            account.littleskin_access_token = account.littleskin_access_token.as_deref().map(|v| decrypt_field(v).unwrap_or_else(|_| v.to_string())).or(account.littleskin_access_token);
    ...
    pub fn insert(&self, account: &Account) -> Result<(), AppError> {
        let mut conn = self.conn.lock();
        let enc_access_token = account.access_token.as_deref().map(|v| encrypt_field(v)).transpose()?;
        let enc_refresh_token = account.refresh_token.as_deref().map(|v| encrypt_field(v)).transpose()?;
        let enc_littleskin_access_token = account.littleskin_access_token.as_deref().map(|v| encrypt_field(v)).transpose()?;
        db_op!(conn, conn.execute_params(
            ...
            &[&account.id as &dyn rusqlite::types::ToSql, &account.account_type, &account.username, &account.uuid, &enc_access_token as &dyn rusqlite::types::ToSql, &enc_refresh_token, &account.expires_at, &account.skin_url, &account.avatar_url, &account.littleskin_server_url, &enc_littleskin_access_token]
        ))?;
        Ok(())
    }
    ...
    pub fn update_tokens(&self, id: &str, access_token: Option<&str>, refresh_token: Option<&str>, expires_at: Option<u64>) -> Result<(), AppError> {
        let mut conn = self.conn.lock();
        let enc_access_token = access_token.map(|v| encrypt_field(v)).transpose()?;
        let enc_refresh_token = refresh_token.map(|v| encrypt_field(v)).transpose()?;
        db_op!(conn, conn.execute_params(
            "UPDATE accounts SET access_token = ?1, refresh_token = ?2, expires_at = ?3, updated_at = datetime('now') WHERE id = ?4",
            &[&enc_access_token as &dyn rusqlite::types::ToSql, &enc_refresh_token, &expires_at, &id]
        ))?;
        Ok(())
    }
}
...
impl ServerRepository {
    ...
    pub fn get_all(&self) -> Result<Vec<ServerEntry>, AppError> {
        ...
        let rows = db_op!(stmt, stmt.query_map([], |row| {
            ...
            let _mod_list: Option<Vec<String>> = mod_list_str.as_ref().and_then(|s| serde_json::from_str(s).ok());
...
pub fn export_accounts(conn: &mut DatabaseConnection) -> Result<Vec<Account>, AppError> {
    ...
    let mut accounts = Vec::new();
    for row in rows {
        let mut account = row.map_err(|e| errors::database_error("row", e.to_string()))?;
        account.access_token = account.access_token.as_deref().map(|v| decrypt_field(v).unwrap_or_else(|_| v.to_string())).or(account.access_token);
        account.refresh_token = account.refresh_token.as_deref().map(|v| decrypt_field(v).unwrap_or_else(|_| v.to_string())).or(account.refresh_token);
        account.littleskin_access_token = account.littleskin_access_token.as_deref().map(|v| decrypt_field(v).unwrap_or_else(|_| v.to_string())).or(account.littleskin_access_token);
        accounts.push(account);
