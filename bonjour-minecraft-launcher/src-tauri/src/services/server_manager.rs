...
lazy_static::lazy_static! {
    ...
    pub static ref LOCAL_SERVER_CHILDREN: Mutex<HashMap<String, Child>> = Mutex::new(HashMap::new());
    pub static ref LOCAL_SERVER_STDIN: Mutex<HashMap<String, ChildStdin>> = Mutex::new(HashMap::new());
...
pub fn start_server_process(app_handle: Option<tauri::AppHandle>, server_id: &str, server_dir: &str, jar_name: &str, _port: u16, max_memory_mb: u32) -> Result<u32, AppError> {
    ...
    if let Some(sin) = stdin {
        if let Ok(mut map) = LOCAL_SERVER_STDIN.lock() {
            map.insert(server_id.to_string(), sin);
        }
    }

    if let Ok(mut map) = LOCAL_SERVER_CHILDREN.lock() {
        ...
        std::thread::spawn(move || {
            ...
            for line in reader.lines().flatten() {
                if let Some(ref app) = app {
                    let _ = app.emit("server-console-log", serde_json::json!({
                        "serverId": sid_clone,
                        "line": line,
                        "stream": "stdout"
                    }));
...
pub fn send_server_command(server_id: &str, command: &str) -> Result<bool, AppError> {
    if let Ok(mut map) = LOCAL_SERVER_STDIN.lock() {
        if let Some(stdin) = map.get_mut(server_id) {
            writeln!(stdin, "{}", command)
                .map(|_| true)
                .map_err(|e| errors::process_error(None, format!("发送命令失败: {}", e)))
        } else {
            Err(errors::server_error(server_id, "服务器标准输入未找到"))
        }
    } else {
        Err(errors::server_error(server_id, "无法访问服务器进程"))
    }
}
...
pub fn sync_resource_pack(_server_id: &str, url: &str, hash: Option<&str>) -> Result<ServerResourcePackInfo, AppError> {
