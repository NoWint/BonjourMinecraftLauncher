...
impl MicrosoftAuthService {
    ...
    pub async fn refresh_token(&self, refresh_token: &str) -> Result<Account, AppError> {
        let resp = self.client
            .post(TOKEN_URL)
            .form(&[
                ("client_id", get_client_id()),
            ...
            .send()
            .await
            .map_err(|e| crate::errors::internal(format!("Token refresh failed: {}", e)))?;

        let data: serde_json::Value = resp.json().await
            .map_err(|e| crate::errors::internal(format!("Failed to parse refresh response: {}", e)))?;

        if let Some(error) = data["error"].as_str() {
            return Err(crate::errors::internal(format!(
                "Token refresh error: {}",
                data["error_description"].as_str().unwrap_or(error)
            )));
        }

        let new_access_token = data["access_token"].as_str().unwrap_or("");
        let new_refresh_token = data["refresh_token"].as_str().unwrap_or(refresh_token);

        let profile = self.complete_minecraft_auth(new_access_token).await?;
