...
pub fn run() {
    ...
    let app = tauri::Builder::default()
        ...
        .manage(commands::auth::AuthState::new())
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            services::window_manager::window_open_mods_browser,
            services::window_manager::window_open_map_preview,
            services::window_manager::window_open_crash_report,
            services::window_manager::window_emit_to,
            // Batch K: Overlay
        ...
        .setup(|app| {
                ...
                Err(e) => {
                    ...
                    std::thread::spawn(move || {
                        let _ = app_handle.dialog()
                            .message(format!("数据库初始化失败，应用无法继续运行。\n\n错误信息：{}\n\n请尝试删除数据库文件后重新启动。", e))
                            .title("Bonjour Minecraft - 严重错误")
            ...
            let current_lang = std::env::var("BONJOUR_LANG").unwrap_or_else(|_| "zh-CN".to_string());
            let (quick_launch_text, show_text, quit_text) = match current_lang.as_str() {
                "en-US" => ("Quick Launch", "Show Window", "Quit"),
                "ja-JP" => ("クイック起動", "ウィンドウを表示", "終了"),
                "ko-KR" => ("빠른 실행", "창 보기", "종료"),
                _ => ("快速启动", "显示窗口", "退出"),
            };

            let menu = tauri::menu::MenuBuilder::new(app)
                .item(&tauri::menu::MenuItem::with_id(app, "quick-launch", quick_launch_text, true, None::<&str>)?)
                .separator()
                .item(&tauri::menu::MenuItem::with_id(app, "show", show_text, true, None::<&str>)?)
                .separator()
                .item(&tauri::menu::MenuItem::with_id(app, "quit", quit_text, true, None::<&str>)?)
                .build()?;
...
fn cleanup_temp_files() {
    ...
    if temp_dir.exists() {
        match std::fs::remove_dir_all(&temp_dir) {
            Ok(_) => eprintln!("Cleaned up leftover temp files: {}", temp_dir.display()),
            Err(e) => eprintln!("Failed to clean temp files {}: {}", temp_dir.display(), e),
        }
    }
}
