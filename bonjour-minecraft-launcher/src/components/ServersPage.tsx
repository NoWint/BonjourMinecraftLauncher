    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="p-6 space-y-4">
      ...
      {servers.length === 0 ? (
        <EmptyState
          icon={Server}
          title="还没有添加服务器"
          action={{ label: '添加第一个服务器', onClick: onAddServer }}
        />
      ) : (
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="p-6 space-y-4">
      ...
      {localServers.length === 0 ? (
        <EmptyState
          icon={Terminal}
          title="还没有本地服务器"
          description="创建你的第一个 Minecraft 服务器"
          action={{ label: '立即创建', onClick: onCreate }}
        />
      ) : (
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="p-6 space-y-4">
      ...
      {lanWorlds.length === 0 ? (
        <EmptyState
          icon={Wifi}
          title="未发现局域网世界"
          description="确保你的 Minecraft 世界已开启局域网"
        />
      ) : (
...
function CommunityTab({ servers, onRefresh }: { servers: CommunityServer[]; onRefresh: () => void }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="p-6 space-y-4">
      ...
      {filtered.length === 0 ? (
        <EmptyState icon={Globe} title="没有找到服务器" />
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
