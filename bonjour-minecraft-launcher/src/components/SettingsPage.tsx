...
export default function SettingsPage({ settings, onSave, onVersionsChange, onResetSetup }: SettingsPageProps) {
  ...
  const activeDualTheme = useThemeStore((s) => s.activeTheme)
  const switchDualTheme = useThemeStore((s) => s.switchTheme)
  const isDualSwitching = useThemeStore((s) => s.isSwitching)
  ...
  const handleChange = (field: keyof LauncherSettings, value: unknown) => {
    ...
    if (field === 'maxMemory' && typeof value === 'number') {
      const maxMem = systemInfo?.totalMemory ? Math.floor(systemInfo.totalMemory * 0.8) : 32768
      clampedValue = Math.max(512, Math.min(maxMem, value))
    }
    if (field === 'minMemory' && typeof value === 'number') {
      const maxMem = (formData.maxMemory as number) || 4096
      clampedValue = Math.max(256, Math.min(maxMem, value))
    }
  ...
  const sections: SettingSection[] = [
    ...
    },
    {
      ...
      settings: [
        ...
        { key: 'maxMemory', label: '最大内存 (MB)', type: 'number', min: 512, max: systemInfo?.totalMemory ? Math.floor(systemInfo.totalMemory * 0.8) : 32768 },
        { key: 'minMemory', label: '最小内存 (MB)', type: 'number', min: 256, max: (formData.maxMemory as number) || 4096 },
      ],
    },
    {
      id: 'paths',
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 overflow-auto px-8 pb-8">
        <div className="max-w-lg mx-auto space-y-4">
          ...
          <div className="panel">
            <h3 className="text-sm font-medium text-theme-muted mb-3 uppercase tracking-wider">双主题</h3>
            <div className="flex items-center justify-between">
              <span className="text-sm text-theme-secondary">风格切换</span>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => switchDualTheme('cassette')}
                  disabled={isDualSwitching}
                  className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium transition-all"
                  style={{
                    background: activeDualTheme === 'cassette' ? 'var(--accent-dim)' : 'transparent',
                    color: activeDualTheme === 'cassette' ? 'var(--accent)' : 'var(--text-muted)',
                    border: activeDualTheme === 'cassette' ? '1px solid var(--accent)' : '1px solid transparent',
                ...
                <button
                  onClick={() => switchDualTheme('wabi-sabi')}
                  disabled={isDualSwitching}
                  className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium transition-all"
                  style={{
                    background: activeDualTheme === 'wabi-sabi' ? 'var(--accent-dim)' : 'transparent',
                    color: activeDualTheme === 'wabi-sabi' ? 'var(--accent)' : 'var(--text-muted)',
                    border: activeDualTheme === 'wabi-sabi' ? '1px solid var(--accent)' : '1px solid transparent',
                    borderRadius: 'var(--radius-sm)',
                  }}
                >
                  <span>🍵</span>
          ...
          <div className="glass rounded-2xl p-4">
            ...
            {onResetSetup && (
              <button
                ...
                className="mt-2 w-full flex items-center justify-center gap-2 px-3 py-2 rounded-xl transition-all text-sm"
                style={{
