...
function DefaultAnimation({ phase, versionName, prefersReduced }: { phase: AnimationPhase; versionName: string; prefersReduced: boolean }) {
  return (
    <div className="relative flex flex-col items-center gap-6">
      <motion.div
        ...
        initial={{ opacity: 1 }}
        animate={prefersReduced ? { opacity: phase === 'launch' ? [1, 0.7, 1] : 1 } : {
          scale: phase === 'shrink' ? 0.8 : phase === 'transition' ? 1.2 : 1,
          opacity: phase === 'launch' ? [1, 0.5, 1] : 1,
        }}
        transition={{
          duration: phase === 'launch' ? (prefersReduced ? 2 : 0.6) : 0.4,
      ...
      <motion.div
        ...
        animate={{
          ...
          y: prefersReduced ? 0 : (phase === 'transition' ? 0 : -10),
        }}
      ...
      <motion.div
        ...
        animate={{
          ...
          scaleX: prefersReduced ? 1 : (phase === 'launch' ? 1 : 0),
        }}
        transition={{ duration: 0.3 }}
      >
        <motion.div
...
function MinimalAnimation({ phase, versionName, prefersReduced }: { phase: AnimationPhase; versionName: string; prefersReduced: boolean }) {
  return (
    <div className="relative flex flex-col items-center gap-4">
      ...
      >
        <motion.div
          ...
          style={{ background: 'var(--accent)' }}
          animate={prefersReduced ? { opacity: phase === 'launch' ? [1, 0.5, 1] : 1 } : {
...
function CinematicAnimation({ phase, versionName, prefersReduced }: { phase: AnimationPhase; versionName: string; prefersReduced: boolean }) {
  return (
    <div className="relative flex flex-col items-center gap-8">
      <motion.div
        className="relative"
        initial={prefersReduced ? { opacity: 0 } : { scale: 0, rotate: -180 }}
        animate={prefersReduced ? { opacity: phase !== 'idle' ? 1 : 0 } : {
          scale: phase === 'shrink' ? 0.5 : phase === 'transition' ? [0.5, 1.5] : phase === 'launch' ? 1 : 0,
          rotate: phase === 'transition' ? [-180, 0] : 0,
        }}
        transition={prefersReduced ? { duration: 0.3 } : { type: 'spring', stiffness: 200, damping: 20 }}
      >
        <div
      ...
      <motion.div
        ...
        animate={{
          ...
          y: prefersReduced ? 0 : (phase === 'transition' ? 0 : 20),
        }}
...
function RetroAnimation({ phase, versionName, prefersReduced }: { phase: AnimationPhase; versionName: string; prefersReduced: boolean }) {
  return (
...
export default function LaunchAnimation({
  ...
  useEffect(() => {
    if (isLaunching && phase === 'idle') {
      ...
      const timers = [
        ...
        setTimeout(() => {
          if (showCelebration && !prefersReduced) {
            setCelebrating(true)
  ...
  }, [isLaunching, phase, onComplete, showCelebration, prefersReduced])
        ...
        {phase !== 'idle' && phase !== 'complete' && (
          ...
          >
            <motion.div
              ...
              initial={{ opacity: 0 }}
              animate={{
                opacity: prefersReduced ? (phase === 'transition' ? 0.6 : 1) : (phase === 'shrink' ? 0.3 : phase === 'transition' ? 0.6 : 1),
              }}
              transition={{ duration: 0.4 }}
            />
            <AnimationComponent phase={phase} versionName={versionName} prefersReduced={prefersReduced} />
          </motion.div>
        )}
      </AnimatePresence>
