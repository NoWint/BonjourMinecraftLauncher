...
import { Palette, Plus, Download, Upload, Trash2, Check, Sun, Moon, Monitor, Sparkles, Eye, Copy, Accessibility } from 'lucide-react'
import { useTheme, type ThemePreset, type ThemeColors, BUILT_IN_PRESETS } from '../hooks/themeContext'

const COLOR_BLIND_PRESETS: readonly { id: string; name: string; desc: string; accent: string; icon: string; filter?: string }[] = [
  {
    id: 'none',
    name: '无',
    desc: '正常色觉',
...
export default function ThemeEditor() {
  ...
  const [colorBlindMode, setColorBlindMode] = useState<string>(() => {
    return localStorage.getItem('a11y-colorBlindMode') || 'none'
  })

  const darkPresets = presets.filter(p => p.mode === 'dark')
  const lightPresets = presets.filter(p => p.mode === 'light')

  const handleColorBlindChange = useCallback((modeId: string) => {
    setColorBlindMode(modeId)
    localStorage.setItem('a11y-colorBlindMode', modeId)
    const root = document.documentElement
    const preset = COLOR_BLIND_PRESETS.find(p => p.id === modeId)
    if (preset && preset.filter) {
      root.style.setProperty('--color-blind-filter', preset.filter)
      root.setAttribute('data-color-blind', modeId)
    } else {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass rounded-2xl p-4">
        ...
        <p className="text-xs text-theme-muted mb-3">为不同色觉类型优化配色方案，调整强调色以提高辨识度</p>
        <div className="grid grid-cols-1 gap-2">
          {COLOR_BLIND_PRESETS.map(preset => (
            <button
              key={preset.id}
              onClick={() => handleColorBlindChange(preset.id)}
              className={`flex items-center gap-3 p-3 rounded-xl transition-all text-left ${
                colorBlindMode === preset.id
                  ? 'ring-2 ring-mc-green bg-white/5 dark:bg-white/5'
                  : 'hover:bg-white/5 dark:hover:bg-white/5'
              }`}
            >
              <span className="text-lg flex-shrink-0" aria-hidden="true">{preset.icon}</span>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="text-sm font-medium text-theme-primary">{preset.name}</span>
                  {colorBlindMode === preset.id && (
                    <Check className="w-3.5 h-3.5 text-mc-green" />
                  )}
                </div>
                <p className="text-xs text-theme-muted mt-0.5">{preset.desc}</p>
              </div>
              {preset.accent && (
                <div className="w-6 h-6 rounded-full flex-shrink-0 border border-white/10" style={{ background: preset.accent }} />
              )}
            </button>
          ))}
        </div>
