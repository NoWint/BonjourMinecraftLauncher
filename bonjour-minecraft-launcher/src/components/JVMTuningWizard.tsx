...
import { X, Cpu, AlertTriangle, CheckCircle, Info } from 'lucide-react'
...
export default function JVMTuningWizard({
  ...
  currentMinMemory: _currentMinMemory = 512,
  ...
  const [_step, _setStep] = useState<'select' | 'customize' | 'apply'>('select')

  useEffect(() => {
    if (window.minecraftAPI?.getJVMProfiles) {
      window.minecraftAPI.getJVMProfiles().then((p) => {
        setProfiles(p as JVMProfile[])
      })
    }
  }, [])
