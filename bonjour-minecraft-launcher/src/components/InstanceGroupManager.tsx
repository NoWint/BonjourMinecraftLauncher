...
import { X, FolderPlus, Tag, Trash2, ChevronRight, ChevronDown, Folder } from 'lucide-react'
...
export default function InstanceGroupManager({ onClose, onAssignGroup: _onAssignGroup, onAssignTag: _onAssignTag, selectedInstanceId: _selectedInstanceId }: InstanceGroupManagerProps) {
  ...
  useEffect(() => {
    window.minecraftAPI?.getInstanceGroups().then((c) => {
      setConfig(c as GroupConfig)
    })
  }, [])
  ...
  const handleCreateTag = async () => {
    ...
    if (tag && config) {
      setConfig({ ...config, tags: [...config.tags, tag as any] })
    }
    setNewTagName('')
    setShowNewTag(false)
  }
