...
function LanguageSettings() {
  ...
  const handlePreviewLanguage = useCallback((code: string) => {
  ...
  const handleCancelPreview = useCallback(() => {
    if (previewLang && previewLang !== currentLang) {
      i18n.changeLanguage(currentLang)
    }
    setPreviewLang(null)
  }, [i18n, currentLang, previewLang])

  return (
    <div className="space-y-6">
      {previewLang && previewLang !== currentLang && (
        <motion.div
          initial={{ opacity: 0, y: -10 }}
          animate={{ opacity: 1, y: 0 }}
          className="glass rounded-2xl p-4 border border-amber-500/20 bg-amber-500/5"
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Globe className="w-4 h-4 text-amber-400" />
              <span className="text-sm text-amber-300">语言预览中</span>
            </div>
            <div className="flex gap-2">
              <button onClick={handleCancelPreview}
                className="px-3 py-1.5 text-xs rounded-lg text-white/40 hover:text-white hover:bg-white/5 transition-colors">
                取消
              </button>
              <button onClick={() => { setCurrentLang(previewLang); setPreviewLang(null) }}
                className="px-3 py-1.5 text-xs rounded-lg bg-mc-green/15 text-mc-green hover:bg-mc-green/25 transition-colors">
                确认切换
              </button>
            </div>
          </div>
        ...
        <div className="space-y-2">
          {LANGUAGE_OPTIONS.map(lang => (
            <button
              ...
              onMouseEnter={() => handlePreviewLanguage(lang.code)}
              onMouseLeave={() => { if (previewLang) handleCancelPreview() }}
              className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all"
      ...
      <div className="glass rounded-2xl p-4">
        <p className="text-xs text-theme-muted">
          💡 鼠标悬停在语言选项上可预览效果，点击确认切换。部分界面文本可能尚未翻译完成。
        </p>
      </div>
    </div>
  )
