...
import { Zap, Clock, TrendingUp, TrendingDown, Minus, BarChart3, Play, Settings, Star } from 'lucide-react'
...
export default function LaunchBenchmarkPanel({ version, onQuickLaunch }: LaunchBenchmarkPanelProps) {
  ...
  useEffect(() => {
    window.minecraftAPI?.getBenchmarkSummary?.(version).then((s: BenchmarkSummary | null) => {
      setSummary(s)
    })
    window.minecraftAPI?.getLaunchBenchmarks?.(version, 10).then((r: BenchmarkRecord[]) => {
      setRecords(r)
    })
  }, [version])
