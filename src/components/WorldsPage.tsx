    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 overflow-auto px-6 pb-6">
        ...
        {activeTab === 'worlds' && (
          isLoading ? (
            ...
            <div className="text-center py-20">
              ...
              <p className="text-white/40 text-sm">{searchQuery ? '未找到匹配的存档' : '还没有存档'}</p>
              <p className="text-xs text-white/30 mt-1">启动游戏创建新世界后，存档将自动出现在这里</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        {activeTab !== 'worlds' && !selectedWorld && activeTab !== 'seed' && (
          <div className="text-center py-20">
            <Globe className="w-10 h-10 text-white/20 mx-auto mb-3" />
            <p className="text-white/40 text-sm">请先选择一个存档</p>
            <button onClick={() => setActiveTab('worlds')}
              className="mt-4 px-4 py-2 rounded-xl bg-mc-green/15 border border-mc-green/25 text-mc-green text-sm font-medium hover:bg-mc-green/25 transition-all">
              返回存档列表
            </button>
          </div>
        )}
          ...
          <div className="space-y-4">
            ...
            {timeline && timeline.entries.length === 0 ? (
              <div className="text-center py-16">
                ...
                <p className="text-white/40 text-sm">暂无时间线记录</p>
                <p className="text-xs text-white/30 mt-1">创建快照后，存档状态将记录在时间线上</p>
              </div>
            ) : timeline && (
