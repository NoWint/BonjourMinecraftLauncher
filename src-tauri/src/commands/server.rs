...
#[tauri::command]
pub async fn start_local_server(server_id: String) -> Result<serde_json::Value, AppError> {
    let mut local_servers = sm::load_local_servers();
    ...
    match sm::start_server_process(&server.server_dir, &jar_name, server.port, max_memory) {
        Ok(pid) => {
            server.pid = Some(pid);
