        ...
        if let Ok(entries) = std::fs::read_dir(path) {
            for entry in entries.flatten() {
                ...
                if name.ends_with(".jar") || name.ends_with(".jar.disabled") {
                    ...
                    let authors = metadata.as_ref().and_then(|m| m.authors.clone());
                    let dependencies = metadata.as_ref().and_then(|m| m.dependencies.clone());
...
pub async fn search_mods_global(
    ...
    let mut modrinth_info = ModSourceInfo::default();
    let mut curseforge_info = ModSourceInfo::default();
